...
def extract_frames(target_path: str, temp_video_resolution: str, temp_video_fps: Fps) -> bool:
    ...
    # Add hardware acceleration for decoding (CUDA)
    commands = ['-hwaccel', 'cuda', '-i', target_path, '-s', str(temp_video_resolution), '-q:v', '0']
...
def merge_video(target_path: str, output_video_resolution: str, output_video_fps: Fps) -> bool:
    ...
    # Use NVENC hardware acceleration for encoding
    commands = ['-r', str(temp_video_fps), '-i', temp_frames_pattern, '-s', str(output_video_resolution)]

    # Choose the correct encoder: NVENC if specified
    if facefusion.globals.output_video_encoder in ['libx264', 'libx265']:
        output_video_compression = round(51 - (facefusion.globals.output_video_quality * 0.51))
        commands.extend(['-c:v', facefusion.globals.output_video_encoder, '-crf', str(output_video_compression), '-preset', facefusion.globals.output_video_preset])
    elif facefusion.globals.output_video_encoder in ['h264_nvenc', 'hevc_nvenc']:
        # NVENC-specific settings
        output_video_compression = round(51 - (facefusion.globals.output_video_quality * 0.51))
        commands.extend(['-c:v', facefusion.globals.output_video_encoder, '-cq', str(output_video_compression), '-preset', map_nvenc_preset(facefusion.globals.output_video_preset)])
    else:
        raise ValueError("Unsupported video encoder specified.")
...
def map_nvenc_preset(output_video_preset: OutputVideoPreset) -> Optional[str]:
    ...
    if output_video_preset == 'medium':
        return 'medium'
    if output_video_ppreset in ['slow', 'slower', 'veryslow']:
        return 'slow'
    return None
